...
// Mock fetch
let fetchOut: any;
jest.spyOn(<any> globalThis, 'fetch').mockImplementation((_input: any, _init: any) => {
  return fetchOut;
});

function getMock(options: { statusCode?: number; error?: boolean; body?: Readable; headers?: any; url?: string }) {
  if (options.error) {
...
function mockSetup(...args: Parameters<typeof getMock>) {
...
describe('dereferencer', () => {
  ...
  it('should error on 404 responses', async() => {
    ...
    await expect(rdfDereferencer.dereference('http://example.org/')).rejects
...
`);
    ...
    expect(out.url).toBe('http://example.org/');
    await expect(arrayifyStream(out.data)).resolves.toBeRdfIsomorphic([
...
`);
    ...
    const myFetch: typeof fetch = async _url =>
      getMock({ statusCode: 200, body, headers: { 'content-type': 'text/turtle' }}) as any;
...
`);
    ...
    expect(out.metadata.triples).toBeTruthy();
    expect(out.url).toBe('http://example.org/bla.ttl');
    await expect(arrayifyStream(out.data)).resolves.toBeRdfIsomorphic([
      quad('http://ex.org/s', 'http://ex.org/p', 'http://ex.org/o1'),
      quad('http://ex.org/s', 'http://ex.org/p', 'http://ex.org/o2'),
    ]);
...
`);
    ...
    expect(out.metadata.triples).toBeTruthy();
    expect(out.url).toBe('http://example.org/');
    await expect(arrayifyStream(out.data))
...
`);
    ...
    expect((<any> mocked(fetch).mock.calls[0][1].headers).get('_a')).toBe('A');
    expect((<any> mocked(fetch).mock.calls[0][1].headers).get('_b')).toBe('B');
    expect(out.headers).toEqual(new Headers({
      'content-type': 'text/turtle',
    }));
  });
...
`);
  ...
  it('should handle absolute local .shaclc files', async() => {
    const out = await rdfDereferencer.dereference(
      join(process.cwd(), 'test/assets/example.shaclc'),
      { localFiles: true },
    );
    expect(out.metadata.triples).toBeTruthy();
